...
export const getStocksAfterBought = (
  currentStock: Stock,
  stocks: Stock[],
  amount: number,
) => {
  const newStocks: Stock[] = stocks;
  const prevBoughtStocks =
    newStocks[newStocks.findIndex((stock) => stock.id === currentStock.id)]
      .bought;
  newStocks[
    newStocks.findIndex((stock) => stock.id === currentStock.id)
  ].bought = prevBoughtStocks + amount;
  return newStocks;
};

export const getStocksAfterSelling = (
  currentStock: Stock,
  stocks: Stock[],
  amount: number,
) => {
