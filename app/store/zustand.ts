...
export const useMoneyStore = create<MoneyStore>((set) => ({
  ...
  portfolioValue: 0,
  portfolioHistory: [0],
  reduceAvailableMoney: (amount: number) =>
    set((state) => ({ availableMoney: state.availableMoney - amount })),
  increaseAvailableMoney: (amount: number) =>
    set((state) => ({ availableMoney: state.availableMoney + amount })),
  setNewPortfolioValue: (value: number) =>
    set((state) => ({
...
export const useStockStore = create<StocksStore>((set) => ({
  ...
  newValuesForStocks: (stocks: Stock[]) =>
    set((state) => ({
      stocks: [...stocks],
    })),
}));
